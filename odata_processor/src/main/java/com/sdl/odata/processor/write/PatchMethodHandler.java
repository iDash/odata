    ...
    private ProcessorResult processRequest(Object entity) throws ODataException {
        ...
        if (!targetType.isCollection()) {
            ...
            if (dataSource instanceof DataSourcePatchSupport) {
                updatedEntity = ((DataSourcePatchSupport) dataSource).patch(getODataRequestContext(),
                    getEntityDataModel());
                validateProperties(updatedEntity, getEntityDataModel());
                updatedEntity = dataSource.update(getoDataUri(), updatedEntity, getEntityDataModel());
            } else {
